...
def f(t, y, ratio_absorb,PSII_content_per_leaf,pKreg, max_PSII, kQA, max_b6f, lumen_protons_per_turnover, PAR, ATP_synthase_max_turnover, 
    ...
    PSI_antenna_size=1-PSII_antenna_size   
...
def do_stuff(LIGHT):
    ...
    # 保存所有 idx 的结果到一个 CSV 文件
    combined_df.to_csv(f'./logs_QA/combined_{LIGHT}_simulated_PSI.csv', index=False)
    print(f"All results for LIGHT {LIGHT} saved to combined_{LIGHT}_simulated_PSI.csv")
