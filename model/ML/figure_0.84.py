...
for i, ratio in enumerate(unique_ratios):
    ...
    # ax.set_title('Ratio Absorb = {ratio:.2f}', fontsize=12)
    ax.set_title(r'$Abs_{ratio}$ = ' + f'{ratio:.2f}', fontsize=18)
    ax.set_xlabel('$PSII_{antenna}$', fontsize=18)
    ax.set_ylabel('PSI/leaf area', fontsize=18)
...
cbar.set_label('A (µmol $m^{-2} s^{-1}$)', fontsize=14)
